...
// Supports byte-mode encoding, EC level L, versions 1-20
...
function polyMul(p1: number[], p2: number[]): number[] {
  const result: number[] = [];
  for (let i = 0; i < p1.length + p2.length - 1; i++) result.push(0);
...
function getECCodewords(data: number[], ecCount: number): number[] {
  ...
  const padded: number[] = [];
  for (let i = 0; i < data.length + ecCount; i++) padded.push(i < data.length ? data[i] : 0);
...
function interleave(data: number[], version: number): number[] {
  ...
  const ecBlocks = blocks.map(function(b) { return getECCodewords(b, ecPerBlock); });
  const result: number[] = [];
  let maxDataLen = 0;
  for (let i = 0; i < blocks.length; i++)
    if (blocks[i].length > maxDataLen) maxDataLen = blocks[i].length;
...
function createMatrix(version: number) {
  ...
  for (let i = 0; i < size; i++) {
    const mRow: number[] = [];
    const rRow: boolean[] = [];
    for (let j = 0; j < size; j++) { mRow.push(0); rRow.push(false); }
    matrix.push(mRow);
    reserved.push(rRow);
  }
...
function reserveVersionBits(matrix: number[][], reserved: boolean[][], version: number) {
  if (version < 7) return;
  const size = matrix.length;
  for (let i = 0; i < 6; i++)
    for (let j = 0; j < 3; j++) {
...
function applyFormatBits(matrix: number[][], maskPattern: number) {
  const size = matrix.length;
  const formatIndex = 1 * 8 + maskPattern;
...
function maskFn(idx: number, r: number, c: number): boolean {
  switch(idx) {
...
function applyMask(matrix: number[][], reserved: boolean[][], maskIdx: number) {
  ...
  for (let r = 0; r < size; r++)
    for (let c = 0; c < size; c++)
      if (!reserved[r][c] && maskFn(maskIdx, r, c))
        matrix[r][c] ^= 1;
}
...
export function generateQRMatrix(text: string): number[][] {
  ...
  for (let maskIdx = 0; maskIdx < 8; maskIdx++) {
    ...
    addFinderPattern(m.matrix, m.reserved, 0, 0);
    addFinderPattern(m.matrix, m.reserved, 0, size - 7);
    addFinderPattern(m.matrix, m.reserved, size - 7, 0);
    if (version >= 2) {
      ...
      for (let i = 0; i < positions.length; i++)
        for (let j = 0; j < positions.length; j++) {
          ...
          addAlignmentPattern(m.matrix, m.reserved, r, c);
        }
    }
    addTimingPatterns(m.matrix, m.reserved);
    reserveFormatBits(m.matrix, m.reserved);
    reserveVersionBits(m.matrix, m.reserved, version);
    placeData(m.matrix, m.reserved, finalData);
    applyMask(m.matrix, m.reserved, maskIdx);
    applyFormatBits(m.matrix, maskIdx);
    const score = scoreMask(m.matrix);
    if (score < bestScore) {
      bestScore = score;
      bestMatrix = [];
      for (let r = 0; r < m.matrix.length; r++)
        bestMatrix.push(m.matrix[r].slice());
    }
  }
  return bestMatrix!;
}
